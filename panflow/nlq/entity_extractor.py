...
class EntityExtractor:
    ...
    def __init__(self):
        ...
        self.entity_patterns = {
            ...
            "policy_type": {
                ...
                "security_pre_rules": [r"(?:pre|pre-rulebase)\s+security\s+(?:policies|rules)", r"pre\s+security"],
                "security_post_rules": [r"(?:post|post-rulebase)\s+security\s+(?:policies|rules)", r"post\s+security"],
            },
            "context": {
    ...
    def extract(self, query: str) -> Dict[str, Any]:
        ...
        if "object_type" in entities and entities["object_type"] == "address" and any(phrase in normalized_query for phrase in ["address-group", "address group", "address-groups", "address groups"]):
            entities["object_type"] = "address-group"

        if "object_type" in entities and entities["object_type"] == "service" and any(phrase in normalized_query for phrase in ["service-group", "service group", "service-groups", "service groups"]):
