...
from panflow.reporting import generate_unused_objects_report, generate_security_rule_coverage_report
...
def cleanup_unused_objects(
    ...
    object_types: List[str] = typer.Option(
        ["address"], "--type", "-t", help="Types of objects to clean up"
    ),
    exclude_file: Optional[str] = typer.Option(
        None, "--exclude-file", help="JSON file with list of object names to exclude from cleanup"
    ),
    dry_run: bool = ConfigOptions.dry_run(),
    report_file: Optional[str] = typer.Option(
        None, "--report-file", "-r", help="JSON file to save the report of cleaned-up objects"
    ),
    ...
    try:
        ...
        for obj_type in object_types:
            ...
            logger.info(
                f"After applying exclusions: {len(filtered_unused)} objects will be processed."
            )
            ...
            if dry_run:
                ...
                if filtered_unused:
                    ...
                    cleanup_report["cleaned_objects"][obj_type] = [
                        obj["name"] for obj in filtered_unused
                    ]
            else:
                ...
                for obj in filtered_unused:
                    try:
                        if xml_config.delete_object(
                            obj_type, obj["name"], context, **context_kwargs
                        ):
        ...
        if dry_run:
            logger.info(
                f"Dry run summary: Would have cleaned up {sum(len(objs) for objs in cleanup_report['cleaned_objects'].values())} unused objects"
            )
        else:
            logger.info(
                f"Cleanup summary: Removed {cleanup_report['summary']['total_cleaned_up']} unused objects"
            )
...
def cleanup_disabled_policies(
    ...
    vsys: str = ContextOptions.vsys(),
    template: Optional[str] = ContextOptions.template(),
    policy_types: List[str] = typer.Option(
        ["security_rules"], "--type", "-t", help="Types of policies to clean up"
    ),
    exclude_file: Optional[str] = typer.Option(
        None, "--exclude-file", help="JSON file with list of policy names to exclude from cleanup"
    ),
    dry_run: bool = ConfigOptions.dry_run(),
    report_file: Optional[str] = typer.Option(
        None, "--report-file", "-r", help="JSON file to save the report of cleaned-up policies"
    ),
    version: Optional[str] = ConfigOptions.version(),
):
    """
    ...
    try:
        ...
        for policy_type in policy_types:
            ...
            if xml_config.device_type.lower() == "panorama":
                ...
                if device_group:
                    ...
                    pre_disabled_rules = [
                        elem.get("name") for elem in pre_disabled_elements if elem.get("name")
                    ]
                    ...
                    post_disabled_rules = [
                        elem.get("name") for elem in post_disabled_elements if elem.get("name")
                    ]
                ...
                disabled_rules = [
                    elem.get("name") for elem in disabled_elements if elem.get("name")
                ]
            ...
            logger.info(
                f"After applying exclusions: {len(filtered_disabled)} policies will be processed."
            )
            ...
            if dry_run:
                ...
                for rule_name in filtered_disabled:
                    try:
                        ...
                        if xml_config.delete_policy(
                            delete_policy_type, rule_name, context, **context_kwargs
                        ):
        ...
        if dry_run:
            logger.info(
                f"Dry run summary: Would have cleaned up {sum(len(rules) for rules in cleanup_report['cleaned_policies'].values())} disabled policies"
            )
        else:
            logger.info(
                f"Cleanup summary: Removed {cleanup_report['summary']['total_cleaned_up']} disabled policies"
            )
