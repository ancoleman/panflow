...
def process_query(
    ...
    output: Optional[str] = typer.Option(None, "--output", "-o", help="Output file for updates (required for cleanup/modify operations, not needed for view-only queries)"),
    interactive: bool = typer.Option(False, "--interactive", "-i", help="Interactive mode"),
    format: str = typer.Option("text", "--format", "-f", help="Output format (text, json)"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show verbose output"),
    use_ai: bool = typer.Option(True, "--ai/--no-ai", help="Use AI for processing if available"),
    ai_provider: Optional[str] = typer.Option(None, "--ai-provider", help="AI provider to use (openai, anthropic)"),
            ...
            if result["success"]:
                ...
                if "result" in result and result["result"]:
                    ...
                    if isinstance(result_data, dict):
                        ...
                        if "cleaned_objects" in result_data and isinstance(result_data["cleaned_objects"], list):
                        ...
                        elif "cleaned_policies" in result_data and isinstance(result_data["cleaned_policies"], list):
                        ...
                        # For unused object reports
                        elif "unused_objects" in result_data and isinstance(result_data["unused_objects"], list):
                            unused_objects = result_data.get("unused_objects", [])
                            if unused_objects:
                                typer.echo(f"\nFound {len(unused_objects)} unused objects:")
                                for obj in unused_objects:
                                    typer.echo(f"  - {obj}")
                            else:
                                typer.echo("\nNo unused objects found.")
                                
                        # For disabled policy reports
                        elif "disabled_policies" in result_data and isinstance(result_data["disabled_policies"], list):
                            disabled_policies = result_data.get("disabled_policies", [])
                            if disabled_policies:
                                typer.echo(f"\nFound {len(disabled_policies)} disabled policies:")
                            ...
                            for key, value in result_data.items():
                                if key not in ["details"]:  # Skip detailed technical data
                                    if isinstance(value, list) and len(value) > 5:
                                        ...
                                        typer.echo(f"  {key}: {value[:5]} ... ({len(value) - 5} more)")
                                    else:
...
def show_help():
    ...
    typer.echo("  python cli.py nlq query \"cleanup unused objects\" --config config.xml --output updated.xml")
