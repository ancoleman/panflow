...
def split_bidirectional_command(
    ...
    rule_name: str = typer.Option(
        ..., "--rule-name", "-r", help="Name of the bidirectional NAT rule to split"
    ),
    ...
    context: str = typer.Option(
        "vsys", "--context", "-x", help="Context (shared, device_group, vsys)"
    ),
    device_group: Optional[str] = typer.Option(
        None, "--device-group", "--dg", help="Device group name (required for device_group context)"
    ),
    vsys: str = typer.Option("vsys1", "--vsys", "-v", help="VSYS name (required for vsys context)"),
    version: str = typer.Option("10.2", "--version", help="PAN-OS version"),
    ...
    try:
        ...
        logger.info(
            f"Configuration: device_type={device_type_to_use}, context={context}, version={version_to_use}"
        )

        if dry_run:
            logger.info("DRY RUN MODE: No changes will be made")
            logger.info(
                f"Would split rule '{rule_name}' and create a reverse rule named '{rule_name}{reverse_name_suffix}'"
            )
            logger.info(
                f"Zone swap: {zone_swap}, Address swap: {address_swap}, Disable bidirectional: {disable_bidirectional}"
            )
...
def split_all_bidirectional_command(
    ...
    name_filter: Optional[str] = typer.Option(
        None, "--name-filter", "-f", help="Only process rules containing this string in their name"
    ),
    device_type: str = typer.Option(
        "firewall", "--device-type", "-d", help="Device type (firewall or panorama)"
    ),
    ...
    report_file: Optional[str] = typer.Option(
        None, "--report", "-r", help="Save a detailed report of the operation (JSON format)"
    ),
    dry_run: bool = ConfigOptions.dry_run(),
):
    """
    ...
    try:
        ...
        if dry_run:
            ...
            base_xpath = get_policy_xpath(
                policy_type, device_type_to_use, context, version_to_use, **context_kwargs
            )
            ...
            if name_filter:
                rule_xpath = (
                    f"{base_xpath}/entry[bi-directional='yes' and contains(@name, '{name_filter}')]"
                )
            else:
            ...
            for rule in bidirectional_rules:
                ...
                logger.info(
                    f"  - Would split rule '{rule_name}' and create '{rule_name}{reverse_name_suffix}'"
                )
