...
def filter_policies(
    ...
    **kwargs,
    ...
    all_policies = get_policies(tree, policy_type, device_type, context_type, version, **kwargs)
    ...
    logger.info(
        f"Filtered {len(all_policies)} policies down to {len(filtered_policies)} matching policies"
    )
...
def clone_policy(
    ...
    **kwargs,
    ...
    if add_policy(
        tree, policy_type, new_name, policy_data, device_type, context_type, version, **kwargs
    ):
