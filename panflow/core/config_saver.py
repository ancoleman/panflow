...
from .xml.base import parse_xml, prettify_xml, validate_xml, element_to_dict
...
class ConfigSaver:
    ...
    def __init__(
        ...
        self.backup_dir = (
            os.path.abspath(backup_dir) if backup_dir else os.path.join(self.config_dir, "backups")
        )
        ...
        try:
            ...
            if self.validate_before_save and self.schema_file:
                tree = (
                    ...
                    else etree.ElementTree(tree_or_element)
                )
    ...
    def save_as_json(
        ...
        try:
            ...
            element = (
                tree_or_element
                if isinstance(tree_or_element, etree._Element)
                else tree_or_element.getroot()
            )
    ...
    def get_saved_configs(
        self, pattern: Optional[str] = None, include_backups: bool = False
    ) -> List[str]:
        """
