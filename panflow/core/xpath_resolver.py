"""
XPath resolver for PAN-OS XML utilities.
...
def load_xpath_mappings(version: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Loaded XPath mappings for PAN-OS version {version}")
        ...
        error_msg = f"Error loading XPath mappings for version {version}: {e}"
        logger.error(error_msg)
        raise ValueError(error_msg)
    ...
    **kwargs
    ...
    try:
        if context_type == "device_group" and "device_group" in kwargs:
            xpath = xpath_template.format(device_group=kwargs["device_group"])
            logger.debug(f"Generated context XPath for {device_type}/{context_type}: {xpath}")
            return xpath
        elif context_type == "template" and "template" in kwargs:
            xpath = xpath_template.format(template=kwargs["template"])
            logger.debug(f"Generated context XPath for {device_type}/{context_type}: {xpath}")
            return xpath
...
def get_object_xpath(
    ...
    **kwargs
    ...
    if name:
        ...
        logger.debug(f"Generated object XPath for {object_type}: {xpath}")
        return xpath
    else:
        # Return path to all objects of this type without removing the entry part
...
def get_policy_xpath(
    ...
    **kwargs
    ...
    if name:
        xpath = xpath_template.format(base_path=base_path, name=name)
        logger.debug(f"Generated policy XPath for {policy_type}: {xpath}")
        return xpath
    else:
        # Return path to all policies of this type (remove the [@name=''] part)
...
def get_all_versions() -> List[str]:
    ...
    try:
        ...
        logger.error(f"Error scanning for available PAN-OS versions: {e}")
...
def determine_version_from_config(config_xml: str) -> str:
    ...
    try:
        ...
        logger.warning(f"Error detecting PAN-OS version from configuration: {e}")
