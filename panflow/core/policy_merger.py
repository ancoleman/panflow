...
class PolicyMerger:
    ...
    def __init__(
        ...
        logger.info(
            f"PolicyMerger initialized: source={self.source_device_type} (v{self.source_version}), target={self.target_device_type} (v{self.target_version})"
        )
    ...
    def copy_policy(
        ...
        **kwargs,
        ...
        """
        logger.info(
            f"Copying policy: {policy_type}/{policy_name} from {source_context_type} to {target_context_type}"
        )
        logger.debug(
        ...
        if target_elements:
            ...
            if parent is not None:
                ...
                logger.error(
                    f"Cannot remove existing policy '{policy_name}' - parent element not found"
                )
        ...
        try:
            ...
            if self.source_version != self.target_version:
                logger.debug(
                    f"Handling version-specific attributes for {policy_type} '{policy_name}'"
                )
        ...
        try:
            ...
            self._add_policy_at_position(target_parent, new_policy, position, ref_policy_name)
        except Exception as e:
    ...
    def copy_policies(
        ...
        **kwargs,
        ...
        """
        logger.info(
            f"Copying multiple policies of type {policy_type} from {source_context_type} to {target_context_type}"
        )
    ...
    def merge_all_policies(
        ...
        **kwargs,
        ...
        """
        logger.info(
            f"Merging all policies of types {policy_types} from {source_context_type} to {target_context_type}"
        )
        ...
        logger.info(
            f"Overall merge complete: copied {total_copied} of {total_attempted} policies across all types"
        )
    ...
    def _extract_context_params(
        ...
        try:
            ...
            logger.debug(
                f"Extracted context parameters for {context_type} with prefix '{prefix}': {params}"
            )
            return params
        except Exception as e:
            logger.error(
                f"Error extracting context parameters for {context_type}: {e}", exc_info=True
            )
            return params
        ...
        try:
            ...
            for key, value in criteria.items():
                ...
                if key.startswith("xpath:"):
                    ...
                    if not matches:
                        logger.debug(
                            f"Element '{element_name}' does not match XPath criteria: {xpath}"
                        )
                        return False
                    continue
                ...
                if key == "name":
                    ...
                    if value not in tag_values:
                        logger.debug(
                            f"Element '{element_name}' does not have required tag: {value}"
                        )
                        return False
                    ...
                    if isinstance(value, list):
                        ...
                        if not any(v in member_values for v in value):
                            logger.debug(
                                f"Element '{element_name}' {key} does not contain any of: {value}"
                            )
                            return False
                    else:
                        ...
                        if value not in member_values:
                            logger.debug(
                                f"Element '{element_name}' {key} does not contain: {value}"
                            )
                            return False
                else:
                    ...
                    if (
                        ...
                        logger.debug(
                            f"Element '{element_name}' child {key} text mismatch: {child_elements[0].text} != {value}"
                        )
                        return False
        ...
        try:
            if position == "top":
                ...
                if not ref_policy_name:
                    logger.warning(
                        f"Reference policy name is required for '{position}' operation, adding at bottom"
                    )
                    parent.append(policy)
                    return
                ...
                if ref_index is None:
                    logger.warning(
                        f"Reference policy '{ref_policy_name}' not found, adding at bottom"
                    )
                    parent.append(policy)
                    return
            ...
            logger.error(
                f"Error adding policy '{policy_name}' at position {position}: {e}", exc_info=True
            )
    ...
    def _collect_policy_references(self, policy: etree._Element) -> None:
        ...
        try:
                ...
                if profiles_elem is not None:
                    ...
                    for profile_type in security_profile_types:
                        ...
                        if profile_elem is not None and profile_elem.text:
                            ...
                            logger.debug(
                                f"Added {profile_type} profile reference: {profile_elem.text}"
                            )
            ...
            logger.error(
                f"Error collecting references from policy '{policy_name}': {e}", exc_info=True
            )

    def _copy_referenced_objects(
        self, target_context_type: str, source_context_type: str, **kwargs
    ) -> None:
        ...
        source_base = get_context_xpath(
            self.source_device_type, source_context_type, self.source_version, **source_params
        )

        target_base = get_context_xpath(
            self.target_device_type, target_context_type, self.target_version, **target_params
        )
        ...
        for obj_type, xpath_pattern in object_types.items():
            ...
            logger.info(
                f"Processing {len(self.object_references[obj_type])} references of type {obj_type}"
            )
            ...
            if not target_parent_elements:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to create parent path {target_parent_path}: {e}", exc_info=True
                    )
                    continue
            else:
                target_parent = target_parent_elements[0]
    ...
    def _handle_version_specific_attributes(
        self, policy_element: etree._Element, policy_type: str
    ) -> etree._Element:
        """
        ...
        logger.debug(
            f"Handling version-specific attributes for {policy_type} policy '{policy_name}'"
        )

        # Normalize policy type to match keys in VERSION_SPECIFIC_ATTRIBUTES
        if (
            policy_type == "security_pre_rules"
            or policy_type == "security_post_rules"
            or policy_type == "security_rules"
        ):
            policy_type_normalized = "security"
        elif (
            policy_type == "nat_pre_rules"
            or policy_type == "nat_post_rules"
            or policy_type == "nat_rules"
        ):
            policy_type_normalized = "nat"
        elif (
            policy_type == "pbf_pre_rules"
            or policy_type == "pbf_post_rules"
            or policy_type == "pbf_rules"
        ):
            policy_type_normalized = "pbf"
        elif (
            policy_type == "decryption_pre_rules"
            or policy_type == "decryption_post_rules"
            or policy_type == "decryption_rules"
        ):
        ...
        if policy_type_normalized in self.VERSION_SPECIFIC_ATTRIBUTES:
            ...
            if self.source_version > self.target_version:
                ...
                for attr_name, attr_versions in attributes.items():
                    ...
                    if (
                        self.source_version in attr_versions
                        and attr_versions[self.source_version] is not None
                        and (
                            self.target_version not in attr_versions
                            or attr_versions[self.target_version] is None
                        )
                    ):
                        ...
                        for elem in attr_elements:
                            if elem is not None and elem.getparent() is not None:
                                logger.debug(
                                    f"Removing newer attribute '{attr_name}' from {policy_type} '{policy_name}'"
                                )
    ...
    def _handle_11_2_to_10_2_changes(
        self, policy_element: etree._Element, policy_type: str
    ) -> None:
        ...
        if policy_type == "security":
            ...
            for element_name in ["ssl-decrypt-mirror", "url-category-match", "rule-type"]:
                ...
                if elem is not None and elem.getparent() is not None:
                    logger.debug(
                        f"Removing 11.x element '{element_name}' from security rule '{policy_name}'"
                    )
                    elem.getparent().remove(elem)
            ...
            if elem is not None and elem.getparent() is not None:
                logger.debug(
                    f"Removing 11.x element 'tls13-action' from decryption rule '{policy_name}'"
                )
                elem.getparent().remove(elem)

    def _handle_11_2_to_10_1_changes(
        self, policy_element: etree._Element, policy_type: str
    ) -> None:
        ...
        if policy_type == "security":
            ...
            if elem is not None and elem.getparent() is not None:
                logger.debug(
                    f"Removing 10.2+ element 'disable-server-response-inspection' from security rule '{policy_name}'"
                )
                elem.getparent().remove(elem)
            ...
            if fallback_elem is None:
                logger.debug(
                    f"Adding required 'fallback' element for NAT rule '{policy_name}' in 10.1"
                )
            ...
            if elem is not None and elem.getparent() is not None:
                logger.debug(
                    f"Removing 10.2+ element 'symmetric-return-addresses' from PBF rule '{policy_name}'"
                )
                elem.getparent().remove(elem)
            ...
            if elem is not None and elem.getparent() is not None:
                logger.debug(
                    f"Removing 10.2+ element 'ssl-protocol-version-min' from decryption rule '{policy_name}'"
                )
                elem.getparent().remove(elem)

    def _handle_10_2_to_10_1_changes(
        self, policy_element: etree._Element, policy_type: str
    ) -> None:
