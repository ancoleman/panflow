...
from panflow.core.query_language import (
    Query, MatchNode, WhereNode, ReturnNode, PatternNode,
    EntityNode, RelationshipNode, ExpressionNode, BinaryOpNode,
    UnaryOpNode, PropertyAccessNode, LiteralNode
)
...
class QueryExecutor:
    ...
    def _execute_match(self, match_node: MatchNode, contexts: List[QueryContext]) -> List[QueryContext]:
        """
    ...
    def _execute_pattern(self, pattern: PatternNode, contexts: List[QueryContext]) -> List[QueryContext]:
        """
        ...
        for context in contexts:
            ...
            for i, relationship in enumerate(pattern.relationships):
                ...
                for entity_context in entity_contexts:
                    ...
                    rel_contexts = self._execute_relationship(relationship, entity, [entity_context])
    ...
    def _execute_entity(self, entity: EntityNode, contexts: List[QueryContext]) -> List[QueryContext]:
        """
    ...
    def _execute_relationship(self, relationship: RelationshipNode, target_entity: EntityNode,
                            contexts: List[QueryContext]) -> List[QueryContext]:
        """
    ...
    def _find_matching_nodes(self, entity: EntityNode, context: QueryContext) -> List[Tuple[str, Dict]]:
        """
        Find nodes in the graph that match the entity pattern.
    ...
    def _node_matches_entity(self, node_id: str, node_data: Dict, entity: EntityNode) -> bool:
        ...
        # Print debug info
        logger.debug(f"Matching node {node_id} with type {node_type} against entity with labels {entity.labels}")
    ...
    def _find_connected_nodes(self, source_id: str, relationship: RelationshipNode,
                            target_entity: EntityNode, context: QueryContext) -> List[Tuple[str, str, Dict]]:
        """
    ...
    def _execute_where(self, where_node: WhereNode, contexts: List[QueryContext]) -> List[QueryContext]:
        """
    ...
    def _execute_return(self, return_node: ReturnNode, contexts: List[QueryContext]) -> List[Dict[str, Any]]:
        """
