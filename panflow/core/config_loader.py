...
def load_config_from_file(
    file_path: str, version: Optional[str] = None, validate: bool = False
    ...
    try:
        ...
        if validate:
            if root.tag != "config":
                ...
                raise ValueError(
                    "XML root element is not 'config' - not a valid PAN-OS configuration"
                )
        ...
        detected_version = determine_version_from_config(
...
def save_config(tree: etree._ElementTree, output_file: str) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Permission denied when saving configuration to {output_file}: {e}", exc_info=True
        )
        return False
    except IOError as e:
...
def xpath_search(
    tree: etree._ElementTree, xpath: str, namespaces: Optional[Dict[str, str]] = None
) -> List[etree._Element]:
    """
...
def extract_element_data(element: etree._Element) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            f"Error extracting data from element {element_tag}[name='{element_name}']: {e}",
            exc_info=True,
        )
...
def detect_device_type(tree: etree._ElementTree) -> str:
    ...
    try:
        ...
        logger.debug(
            f"Panorama score: {panorama_score}, indicators: {', '.join(panorama_indicators) if panorama_indicators else 'none'}"
        )
        logger.debug(
            f"Firewall score: {firewall_score}, indicators: {', '.join(firewall_indicators) if firewall_indicators else 'none'}"
        )
        ...
        if panorama_score > firewall_score:
            ...
            logger.info(
                f"No clear device type detected (tied scores: {panorama_score}), defaulting to firewall"
            )
            return "firewall"
