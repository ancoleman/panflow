...
class RelationshipNode(QueryNode):
    ...
    def __init__(
        self, variable: Optional[str], types: List[str], properties: Dict[str, Any], direction: str
    ):
        """
...
class QueryParser:
    ...
    def parse(self) -> List[QueryNode]:
        ...
        while not self._is_at_end():
            ...
            if self._check(TokenType.MATCH):
                ...
                raise ValueError(
                    f"Unexpected token {self._peek()} at position {self._peek().position}"
                )
    ...
    def _parse_pattern(self) -> PatternNode:
        ...
        while (
            (
                ...
                and self._peek_next().type == TokenType.LBRACKET
            )
            or self._check(TokenType.ARROW)
            or self._check(TokenType.REVERSE_ARROW)
        ):
    ...
    def _parse_equality(self) -> ExpressionNode:
        ...
        while self._match_value(TokenType.OPERATOR, "==") or self._match_value(
            TokenType.OPERATOR, "!="
        ):
            operator = self._previous().value
    ...
    def _parse_comparison(self) -> ExpressionNode:
        ...
        while (
            ...
            or self._match_value(TokenType.OPERATOR, ">=")
            or self._match_value(TokenType.OPERATOR, "<")
            or self._match_value(TokenType.OPERATOR, "<=")
            or self._match_value(TokenType.OPERATOR, "=~")
        ):
