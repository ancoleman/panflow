...
from .xml.base import clone_element
...
class NATRuleSplitter:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            f"NATRuleSplitter initialized: device_type={device_type}, context_type={context_type}, version={version}"
        )
    ...
    def split_bidirectional_rule(
        ...
        try:
            ...
            logger.error(
                f"Error splitting bidirectional NAT rule '{rule_name}': {str(e)}", exc_info=True
            )
    ...
    def split_all_bidirectional_rules(
        ...
        base_xpath = get_policy_xpath(
            policy_type, self.device_type, self.context_type, self.version, **self.context_kwargs
        )
        ...
        for rule in bidirectional_rules:
            ...
            results["details"].append({"rule_name": rule_name, "result": result})
        ...
        logger.info(
            f"Bidirectional NAT rule splitting complete: {results['succeeded']} succeeded, {results['failed']} failed"
        )
        return results
    ...
    def _configure_reverse_rule(
        ...
        if return_rule_any_any:
            ...
            if address_swap:
                ...
                logger.debug(
                    f"Swapped source and destination addresses for reverse rule '{rule_name}'"
                )
        ...
        if src_translation is not None and dst_translation is not None:
            ...
            if address_swap:
                ...
                logger.debug(
                    f"Swapped source and destination translation for reverse rule '{rule_name}'"
                )
            ...
            if address_swap:
                ...
                logger.debug(
                    f"Converted source translation to destination translation for reverse rule '{rule_name}'"
                )
            ...
            if address_swap:
                ...
                logger.debug(
                    f"Converted destination translation to source translation for reverse rule '{rule_name}'"
                )
    ...
    def _create_adapted_translation(
        self, rule: etree._Element, target_type: str, source_elem: etree._Element
    ) -> None:
        """
...
# Standalone function interfaces for backward compatibility
...
def split_bidirectional_nat_rule(
    ...
    **kwargs,
    ...
    splitter = NATRuleSplitter(
        tree=tree, device_type=device_type, context_type=context_type, version=version, **kwargs
    )
