...
class XmlDiff:
    ...
    def _compare_elements(
        ...
        for tag in common_tags:
            ...
            if all('name' in child.attrib for child in source_children + target_children):
                self._compare_named_children(source_children, target_children, path, detect_moves, ignore_order)
            else:
                # For elements without name attributes, compare by index if counts match
                if len(source_children) == len(target_children):
                    for i, (source_child, target_child) in enumerate(zip(source_children, target_children)):
                        child_path = f"{path}{tag}[{i+1}]/"
                        self._compare_elements(source_child, target_child, child_path, detect_moves, ignore_order)
                else:
                    # Different counts, treat as added/removed
                    for i, source_child in enumerate(source_children):
                        if i < len(target_children):
                            child_path = f"{path}{tag}[{i+1}]/"
                            self._compare_elements(source_child, target_children[i], child_path, detect_moves, ignore_order)
                        else:
                            child_path = f"{path}{tag}[{i+1}]/"
    ...
    def _compare_attributes(self, source: etree._Element, target: etree._Element, path: str) -> None:
        """
