...
def parse_xml_string(
    xml_string: Union[str, bytes], validate: bool = False, schema_file: Optional[str] = None
    ...
    try:
        ...
        if HAVE_LXML:
            ...
            if validate:
                logger.warning(
                    "Validation requires lxml which is not installed. Skipping validation."
                )
...
@cached_xpath
def find_elements(
    root: etree._Element, xpath: str, namespaces: Optional[Dict[str, str]] = None
) -> List[etree._Element]:
    """
...
def merge_elements(
    target: etree._Element, source: etree._Element, overwrite: bool = True
) -> etree._Element:
    """
    ...
    try:
        if target.tag != source.tag:
            raise MergeError(
                f"Cannot merge elements with different tags: {target.tag} and {source.tag}"
            )
...
def dict_to_element(
    root_tag: str, data: Dict[str, Any], parent: Optional[etree._Element] = None
) -> etree._Element:
    """
...
def prettify_xml(element: Union[etree._Element, etree._ElementTree], indent: str = "  ") -> str:
    """
...
def validate_xml(element: Union[etree._Element, etree._ElementTree], schema_file: str) -> bool:
    """
