...
def find_objects_by_name(
    ...
    if use_regex:
        logger.info(
            f"Finding {object_type} objects with names matching pattern '{object_name}' throughout the configuration"
        )
        # Compile the regex pattern with case-insensitive flag
        pattern = re.compile(object_name, re.IGNORECASE)
    else:
        logger.info(
            f"Finding {object_type} objects named '{object_name}' throughout the configuration"
        )
    ...
    for context_type, context_params in contexts:
        try:
            ...
            xpath = get_object_xpath(
                object_type, device_type, context_type, version, object_name, **context_params
            )
            ...
            if not use_regex:
                ...
                if elements:
                    for element in elements:
                        obj_loc = ObjectLocation(
                            object_type, object_name, context_type, element, **context_params
                        )
                ...
                all_elements_xpath = get_object_xpath(
                    object_type, device_type, context_type, version, **context_params
                )
                ...
                if elements:
                    for element in elements:
                        ...
                        if name and pattern.search(name):
                            ...
                            obj_loc = ObjectLocation(
                                object_type, name, context_type, element, **context_params
                            )
                            results.append(obj_loc)
            ...
            logger.warning(
                f"Error searching for {object_type} '{object_name}' in {context_type}: {e}"
            )
        ...
        try:
            ...
            for element in elements:
                if _matches_criteria(element, value_criteria):
                    ...
                    if obj_name:
                        obj_loc = ObjectLocation(
                            object_type, obj_name, context_type, element, **context_params
                        )
                        results.append(obj_loc)
                        logger.debug(f"Found matching {obj_loc}")
            ...
            logger.warning(
                f"Error searching for {object_type} with criteria {value_criteria} in {context_type}: {e}"
            )
...
def find_all_locations(
    ...
    total_locations = sum(
        len(locations) for obj_dict in results.values() for locations in obj_dict.values()
    )
...
def find_duplicate_names(
    tree: etree._ElementTree, device_type: str, version: str
) -> Dict[str, Dict[str, List[ObjectLocation]]]:
    """
...
def find_duplicate_values(
    tree: etree._ElementTree, object_type: str, device_type: str, version: str
