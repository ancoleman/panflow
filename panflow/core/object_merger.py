...
class ObjectMerger:
    ...
    def __init__(
        ...
        self.target_device_type = (
            target_device_type.lower() if target_device_type else source_device_type.lower()
        )
        ...
        logger.debug(
            f"Using same tree for source and target: {self.source_tree is self.target_tree}"
        )
        ...
        **kwargs,
        ...
        if source_context_type not in valid_contexts:
            ...
            self.skipped_objects.append(
                (object_type, object_name, f"Invalid source context: {source_context_type}")
            )
            return False
        ...
        if target_context_type not in valid_contexts:
            ...
            self.skipped_objects.append(
                (object_type, object_name, f"Invalid target context: {target_context_type}")
            )
            return False
        ...
        if source_context_type == "device_group" and "source_device_group" not in kwargs:
            ...
            self.skipped_objects.append(
                (object_type, object_name, "Missing source_device_group parameter")
            )
            return False
        ...
        if target_context_type == "device_group" and "target_device_group" not in kwargs:
            ...
            self.skipped_objects.append(
                (object_type, object_name, "Missing target_device_group parameter")
            )
            return False

        logger.info(
            f"Copying {object_type} object '{object_name}' from {source_context_type} to {target_context_type}"
        )
        logger.debug(
            f"Copy parameters: skip_if_exists={skip_if_exists}, copy_references={copy_references}"
        )
        ...
        try:
            ...
            if validate:
                ...
                if not is_valid:
                    ...
                    self.skipped_objects.append(
                        (object_type, object_name, f"Validation failed: {validation_errors}")
                    )
                    return False
        ...
        try:
            ...
            if target_elements:
                ...
                if parent is not None:
                    ...
                    logger.error(
                        f"Cannot remove existing object '{object_name}' - parent element not found"
                    )
                    self.skipped_objects.append(
                        (object_type, object_name, "Cannot remove existing object")
                    )
                    return False
        ...
        try:
            ...
            if len(parent_parts) < 2:
                ...
                logger.error(error_msg)
                self.skipped_objects.append(
                    (object_type, object_name, "Invalid target parent path")
                )
                return False
            ...
            if not target_parent_elements:
                ...
                if target_parent is None:
                    ...
                    logger.error(error_msg)
                    self.skipped_objects.append(
                        (object_type, object_name, "Failed to create target parent path")
                    )
                    return False
            else:
        ...
        try:
            ...
            if self.source_version != self.target_version:
                logger.debug(
                    f"Handling version-specific attributes for {object_type} '{object_name}'"
                )
        ...
        try:
            ...
            self._copy_related_tags(
                source_object, source_context_type, target_context_type, skip_if_exists, **kwargs
            )
        except Exception as e:
        ...
        if copy_references and object_type.endswith("_group"):
            try:
                ...
                logger.warning(
                    f"Error copying group members for '{object_name}': {e}", exc_info=True
                )
                # Continue since the main object was copied successfully
        ...
        if copy_references and object_type == "address_group":
            if find_element(source_object, "./dynamic/filter"):
                try:
                    ...
                    logger.warning(
                        f"Error handling dynamic address group '{object_name}': {e}", exc_info=True
                    )
        ...
        if copy_references and object_type == "service":
            try:
                logger.debug(
                    f"Processing service object '{object_name}' for complex configurations"
                )
                ...
                logger.warning(
                    f"Error handling complex service object '{object_name}': {e}", exc_info=True
                )
        ...
        if copy_references and object_type in [
            "security_profile_group",
            ...
            try:
                ...
                logger.warning(
                    f"Error handling security profile group '{object_name}': {e}", exc_info=True
                )
        ...
        if copy_references and object_type in [
            ...
            try:
                ...
                logger.warning(
                    f"Error handling security profile '{object_name}': {e}", exc_info=True
                )
        ...
        if copy_references and object_type in ["security_rule", "security-rule", "security_policy"]:
            try:
                logger.debug(
                    f"Processing security rule '{object_name}' for security profile references"
                )
                ...
                logger.warning(
                    f"Error handling security profiles in rule '{object_name}': {e}", exc_info=True
                )
        ...
        if copy_references and object_type in ["schedule"]:
            try:
                ...
                logger.warning(
                    f"Error handling schedule object '{object_name}': {e}", exc_info=True
                )
        ...
        if copy_references and object_type in ["custom-url-category", "url-category"]:
            try:
                ...
                logger.warning(
                    f"Error handling custom URL category '{object_name}': {e}", exc_info=True
                )
    ...
    def copy_object_with_dependencies(
        ...
        **kwargs,
        ...
        dependencies = self.analyze_dependencies(
            object_type, object_name, source_context_type, include_policies, **source_kwargs
        )
        ...
        if include_referenced_by:
            for ref_type, ref_name in dependencies["referenced_by"]:
                ...
                if not include_policies and any(
                    ref_type.endswith(policy_type) for policy_type in ["rule", "policy"]
                ):
                    continue
                ...
                if (ref_type, ref_name) not in processed_objects:
                    ...
                    if not success:
                        logger.warning(
                            f"Failed to copy referencing object: {ref_type} '{ref_name}'"
                        )
    ...
    def copy_objects(
        ...
        **kwargs,
        ...
        """
        logger.info(
            f"Copying multiple {object_type} objects from {source_context_type} to {target_context_type}"
        )
        ...
        if object_names:
            ...
            logger.warning(
                "No object names or filter criteria specified, will attempt to copy all objects"
            )
        ...
        for obj in objects_to_copy:
            ...
            if copy_with_dependencies:
                ...
                if dependencies:
                    ...
                    logger.info(
                        f"Dependency analysis for '{name}': {dep_count} dependencies, {ref_count} references"
                    )
            else:
    ...
    def merge_all_objects(
        ...
        **kwargs,
        ...
        """
        logger.info(
            f"Merging multiple object types from {source_context_type} to {target_context_type}"
        )
        ...
        logger.info(
            f"Merged a total of {total_copied} objects out of {total_objects} across all types"
        )
    ...
    def _handle_complex_service_object(
        ...
        **kwargs,
        ...
        if not protocol_elem or not protocol_elem.text:
            logger.debug(
                f"Service '{service_name}' has no protocol defined, skipping complex handling"
            )
            return
    ...
    def _handle_service_application_overrides(
        ...
        **kwargs,
        ...
        if override_elem is not None:
            logger.debug(
                f"Service '{service_name}' has application overrides, ensuring applications exist"
            )
            ...
            if not app_refs:
                logger.debug(
                    f"No application references found in override for service '{service_name}'"
                )
                return
    ...
    def _handle_security_profile(
        ...
        **kwargs,
        ...
        if profile_type == "virus":
            self._handle_antivirus_profile(
                profile_element,
                profile_name,
                source_context_type,
                target_context_type,
            ...
            self._handle_antispyware_profile(
                profile_element,
                profile_name,
                source_context_type,
                target_context_type,
            ...
            self._handle_vulnerability_profile(
                profile_element,
                profile_name,
                source_context_type,
                target_context_type,
            ...
            self._handle_url_filtering_profile(
                profile_element,
                profile_name,
                source_context_type,
                target_context_type,
            ...
            self._handle_file_blocking_profile(
                profile_element,
                profile_name,
                source_context_type,
                target_context_type,
            ...
            self._handle_dns_security_profile(
                profile_element,
                profile_name,
                source_context_type,
                target_context_type,
            ...
            if self.source_version >= "11.0" and self.target_version < "11.0":
                ...
                for ml_elem in ml_elements:
                    ...
                    for elem in elements:
                        logger.debug(
                            f"Removing newer element '{ml_elem}' from antivirus profile '{profile_name}'"
                        )
        ...
        **kwargs,
        ...
        if self.source_version >= "10.2" and self.target_version < "10.2":
            ...
            if cloud_analysis_elem is not None and cloud_analysis_elem.getparent() is not None:
                logger.debug(
                    f"Removing newer element 'cloud-inline-analysis' from anti-spyware profile '{profile_name}'"
                )
        ...
        if self.source_version != self.target_version:
            ...
            if self.source_version >= "10.2" and self.target_version < "10.2":
                ...
                for elem_name in newer_elements:
                    ...
                    if elem is not None and elem.getparent() is not None:
                        logger.debug(
                            f"Removing newer element '{elem_name}' from URL filtering profile '{profile_name}'"
                        )
                        elem.getparent().remove(elem)
            ...
            if self.source_version >= "11.0" and self.target_version < "11.0":
                ...
                if ml_elem is not None and ml_elem.getparent() is not None:
                    logger.debug(
                        f"Removing newer element 'machine-learning-model' from URL filtering profile '{profile_name}'"
                    )
        ...
        if self.source_version != self.target_version:
            ...
            if self.source_version >= "10.2" and self.target_version < "10.2":
                ...
                if phishing_elem is not None and phishing_elem.getparent() is not None:
                    logger.debug(
                        f"Removing newer element 'report-phishing-file' from WildFire profile '{profile_name}'"
                    )
        ...
        **kwargs,
        ...
        if self.source_version != self.target_version:
            ...
            if self.source_version >= "10.2" and self.target_version < "10.2":
                ...
                if cloud_analysis_elem is not None and cloud_analysis_elem.getparent() is not None:
                    logger.debug(
                        f"Removing newer element 'cloud-inline-analysis' from DNS security profile '{profile_name}'"
                    )
                    cloud_analysis_elem.getparent().remove(cloud_analysis_elem)
                    ...
                    for action in ["block", "alert", "allow", "continue", "override"]:
                        ...
                        for member in members:
                            if member.text == category_name:
                                logger.debug(
                                    f"Found reference to custom URL category '{category_name}' in URL filtering profile '{profile_name}'"
                                )

                                # Suggest copying the URL filtering profile
                                logger.info(
                                    f"URL filtering profile '{profile_name}' references this custom URL category. You may want to copy it as well."
                                )
        except Exception as e:
            ...
            for elem_name, profile_type in profile_types:
                ...
                if profile_ref is not None and profile_ref.text:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Error copying {profile_type} profile '{profile_name}': {e}"
                        )
    ...
    def _extract_context_params(
        self, context_type: str, kwargs: Dict[str, Any], prefix: str = ""
    ) -> Dict[str, Any]:
        ...
        for key, value in criteria.items():
            ...
            if key == "name":
                ...
                for value_key in ["ip-netmask", "fqdn", "ip-range"]:
                    ...
                    if elements and elements[0].text:
                        if elements[0].text != value:
                            logger.debug(
                                f"Value mismatch for {value_key}: {elements[0].text} != {value}"
                            )
                            return False
                        matched = True
                        break
                ...
                if (
                    ...
                    logger.debug(
                        f"Child element text mismatch: {child_elements[0].text} != {value}"
                    )
                    return False
        ...
        if static_elements:
            ...
            for member_elem in static_elements:
                if member_elem.text:
                    ...
                    if (member_type, member_name) not in self.merged_objects and (
                        member_type,
                        member_name,
                    ) not in self.referenced_objects:
                        # Track this reference
                        logger.debug(
                            f"Adding to referenced objects list: {member_type} '{member_name}'"
                        )
                        self.referenced_objects.append((member_type, member_name))

                        # Try to copy the member object
                        logger.debug(
                            f"Attempting to copy member object: {member_type} '{member_name}'"
                        )
                        self.copy_object(
                            member_type,
                            member_name,
            ...
            logger.info(
                f"Finished processing {member_count} members of {group_type} '{group_name}'"
            )

    def _handle_version_specific_attributes(
        self, object_element: etree._Element, object_type: str
    ) -> etree._Element:
        """
        ...
        if self.source_version == self.target_version:
            logger.debug(
                f"Source and target versions are the same ({self.source_version}), no attribute transformation needed"
            )
        ...
        if normalized_type not in self.VERSION_SPECIFIC_ATTRIBUTES:
            logger.debug(
                f"No version-specific attribute information available for {normalized_type}"
            )
            return object_element
        ...
        for attr_name, version_info in attr_defs.items():
            ...
            if self.source_version in version_info and self.target_version not in version_info:
                logger.debug(
                    f"Attribute '{attr_name}' exists in source version {self.source_version} but not in target version {self.target_version}"
                )
                ...
                if attr_elements:
                    logger.debug(
                        f"Removing attribute '{attr_name}' from {object_type} '{object_name}'"
                    )
            ...
            # Check if the attribute is required in the target but not in the source
                ...
                logger.debug(
                    f"Attribute '{attr_name}' is required in target version {self.target_version} but not in source version {self.source_version}"
                )
                ...
                if not attr_elements:
                    logger.warning(
                        f"Required attribute '{attr_name}' for target version {self.target_version} is missing in {object_type} '{object_name}'"
                    )
            ...
            if attr_name == "color" and self.source_version != self.target_version:
                ...
                if color_elements:
                    ...
                    if color_element.text and not color_element.text.isdigit():
                        ...
                        if self.source_version > self.target_version:
                            logger.debug(
                                f"Converting named color '{color_element.text}' to default color code '1'"
                            )
    ...
    def _handle_address_version_changes(
        self, address_element: etree._Element, address_name: str
    ) -> None:
    ...
    def validate_object(
        self, object_element: etree._Element, object_type: str, **kwargs
    ) -> Tuple[bool, List[str]]:
        """
        ...
        # Security profile validation
            ...
            self._validate_generic_object(
                object_element, object_name, object_type, validation_errors
            )
        ...
        if is_valid:
            ...
            logger.warning(
                f"Validation failed for {object_type} object '{object_name}' with {len(validation_errors)} errors"
            )
            for error in validation_errors:
                logger.warning(f"  - {error}")
    ...
    def _validate_common_elements(
        self, object_element: etree._Element, object_name: str, validation_errors: List[str]
    ) -> None:
    ...
    def _validate_address_object(
        self, address_element: etree._Element, address_name: str, validation_errors: List[str]
    ) -> None:
        ...
        if not found_type:
            validation_errors.append(
                f"Address object must have one of these types: {', '.join(address_types)}"
            )

    def _validate_address_group(
        self, group_element: etree._Element, group_name: str, validation_errors: List[str]
    ) -> None:
        ...
        if dynamic_elem:
            ...
            if not filter_elem or not filter_elem.text or filter_elem.text.strip() == "":
                ...
                if not self._is_valid_dynamic_filter(filter_elem.text):
                    validation_errors.append(
                        f"Invalid dynamic filter expression: {filter_elem.text}"
                    )

    def _validate_service_object(
        self, service_element: etree._Element, service_name: str, validation_errors: List[str]
    ) -> None:
        ...
        if protocol == "tcp" or protocol == "udp":
            ...
            if not dest_port_elem or not dest_port_elem.text:
                validation_errors.append(
                    f"Service object missing port element for {protocol} protocol"
                )
            ...
            if icmp_type_elem is not None and icmp_type_elem.text:
                try:
                    ...
                    validation_errors.append(
                        f"Invalid ICMP type: {icmp_type_elem.text} (must be a number)"
                    )
            ...
            if icmp_code_elem is not None and icmp_code_elem.text:
                try:
                    ...
                    validation_errors.append(
                        f"Invalid ICMP code: {icmp_code_elem.text} (must be a number)"
                    )
        else:
    ...
    def _validate_tag(
        self, tag_element: etree._Element, tag_name: str, validation_errors: List[str]
    ) -> None:
    ...
    def _validate_edl(
        self, edl_element: etree._Element, edl_name: str, validation_errors: List[str]
    ) -> None:
        ...
        if edl_type not in valid_types:
            validation_errors.append(
                f"Invalid EDL type: {edl_type} (must be one of {', '.join(valid_types)})"
            )
    ...
    def _validate_schedule(
        self, schedule_element: etree._Element, schedule_name: str, validation_errors: List[str]
    ) -> None:
            ...
            if start_time_elem and start_time_elem.text and end_time_elem and end_time_elem.text:
                try:
                    ...
                    if start_time == end_time:
                        validation_errors.append(
                            f"Schedule start time and end time cannot be the same"
                        )
    ...
    def _validate_security_profile_group(
        ...
        if not has_profile:
            validation_errors.append(
                f"Security profile group does not reference any security profiles"
            )

    def _validate_antivirus_profile(
        self, profile_element: etree._Element, profile_name: str, validation_errors: List[str]
    ) -> None:
        ...
        for app_exception in app_exceptions:
            ...
            if not app_name or app_name.strip() == "":
                validation_errors.append(
                    f"Antivirus profile has an application exception with empty application name"
                )
    ...
    def _validate_custom_url_category(
        self, category_element: etree._Element, category_name: str, validation_errors: List[str]
    ) -> None:
        ...
        if type_elem is not None and type_elem.text == "URL List":
            ...
            if list_elem is None:
                ...
                if not members and not external_lists:
                    validation_errors.append(
                        f"URL List custom category has empty list (no members or external lists)"
                    )
                ...
                for member in members:
                    if member.text:
                        ...
                        if not url.startswith(("http://", "https://", "ftp://")) and not "." in url:
                            validation_errors.append(
                                f"Potentially invalid URL format in member: {url}"
                            )
        ...
        if type_elem is not None and type_elem.text == "Category Match":
            ...
            if category_elem is None:
                ...
                if not categories:
                    validation_errors.append(
                        f"Category Match custom category has no category members"
                    )
    ...
    def analyze_dependencies(
        ...
        **kwargs,
        ...
        self._find_object_references(
            object_type, object_name, context_type, result, include_policies, **kwargs
        )
    ...
    def _find_object_references(
        ...
        **kwargs,
        ...
        for ref_type, xpath_pattern in referencing_objects:
            self._find_references_in_objects(
                ref_type, xpath_pattern, object_name, context_type, result, **kwargs
            )
    ...
    def _find_references_in_objects(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.warning(
                f"Error finding references in {ref_object_type} objects: {e}", exc_info=True
            )

    def _find_tag_in_dynamic_filters(
        self, tag_name: str, result: Dict[str, List[Tuple[str, str]]]
    ) -> None:
        ...
        try:
            ...
            if self.source_device_type == "panorama":
                ...
                for dg in device_groups:
                    ...
                    filter_xpath = (
                        f"{dg_xpath}[@name='{dg_name}']/address-group/entry/dynamic/filter"
                    )
                    self._check_filters_for_tag(
                        filter_xpath, tag_name, f"device_group {dg_name}", result
                    )
            ...
            if self.source_device_type == "firewall":
                ...
                for vsys in vsys_entries:
                    ...
                    filter_xpath = (
                        f"{vsys_xpath}[@name='{vsys_name}']/address-group/entry/dynamic/filter"
                    )
                ...
                if filter_elem.text and tag_name in filter_elem.text:
                    ...
                    if parent_entry is not None and parent_entry.tag == "entry":
                        ...
                        if re.search(tag_pattern, filter_text):
                            logger.debug(
                                f"Found tag reference in address group '{group_name}' filter"
                            )
