...
class ConflictResolver:
    ...
    def __init__(self, default_strategy: ConflictStrategy = ConflictStrategy.SKIP):
        ...
        """
        logger.debug(
            f"Initializing ConflictResolver with default strategy: {default_strategy.value}"
        )
    ...
    def resolve_conflict(
        ...
        **kwargs,
        ...
        logger.info(
            f"Resolving conflict for {object_type} '{object_name}' using strategy: {strategy.value}"
        )
        ...
        if strategy == ConflictStrategy.SKIP:
            ...
            return self._rename_strategy(
                source_object, target_object, object_type, object_name, **kwargs
            )
            ...
            return self._keep_target_strategy(
                source_object, target_object, object_type, object_name
            )
        elif strategy == ConflictStrategy.INTERACTIVE:
            logger.warning(
                f"Interactive conflict resolution not implemented, using {self.default_strategy.value} instead"
            )
            return self.resolve_conflict(
                source_object, target_object, object_type, object_name, self.default_strategy
            )
        else:
    ...
    def _keep_newer_strategy(
        ...
        if source_timestamp is None or target_timestamp is None:
            logger.warning(
                f"Cannot determine timestamps for {object_type} '{object_name}', falling back to overwrite strategy"
            )
            return self._overwrite_strategy(source_object, target_object, object_type, object_name)
        ...
        if source_timestamp > target_timestamp:
            ...
            logger.debug(
                f"Target {object_type} '{object_name}' is newer or same age, keeping target"
            )
    ...
    def _merge_address_group(
        self, source_object: etree._Element, target_object: etree._Element
    ) -> None:
    ...
    def _merge_tag(self, source_object: etree._Element, target_object: etree._Element) -> None:
        """Merge tag objects."""
        ...
        if (
            ...
            and source_color.text
            and (target_color is None or not target_color.text)
        ):
    ...
    def _merge_members(self, source_elem: etree._Element, target_elem: etree._Element) -> None:
    ...
    def _merge_filters(
        self, source_dynamic: etree._Element, target_dynamic: etree._Element
    ) -> None:
        ...
        if (
            ...
            and source_filter.text
            and target_filter is not None
            and target_filter.text
        ):
            ...
            source_filter is not None
            and source_filter.text
            and (target_filter is None or not target_filter.text)
        ):
    ...
    def _copy_element_if_empty(
        self, source_object: etree._Element, target_object: etree._Element, xpath: str
    ) -> None:
        ...
        if (
            ...
            and (target_elem is None or not target_elem.text)
        ):
