...
def test_object_merge_workflow(sample_config_path):
    ...
    result = merger.copy_object(
        "address", "shared-address-1", "shared", "shared", skip_if_exists=True, copy_references=True
    )
    ...
    with tempfile.NamedTemporaryFile(suffix=".xml") as tmp:
        ...
        verify_address = find_element(
            verify_tree, './shared/address/entry[@name="shared-address-1"]'
        )
