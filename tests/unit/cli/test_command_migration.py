...
def get_object_original(
    ...
    return original_object_get(
        config, object_type, name, device_type, context, device_group,
        vsys, template, version, output_format, output_file
    )
...
class TestCommandMigration:
    ...
    def test_original_command(self, mock_get_object, mock_load_config, temp_output_file):
        ...
        # Run the command
        result = runner.invoke(migration_app, [
            "get-original",
            "--config", "test.xml",
            "--type", "address",
            "--name", "test-obj",
    ...
    def test_migrated_command(self, mock_get_object, mock_load_config, temp_output_file):
        ...
        with patch('panflow.cli.command_base.CommandBase.load_config', return_value=mock_load_config.return_value):
            # Run the command
            result = runner.invoke(migration_app, [
                "get-migrated",
                "--config", "test.xml",
    ...
    def test_behavior_equivalence(self, mock_get_object, mock_load_config):
        ...
        with patch('panflow.cli.command_base.CommandBase.load_config', return_value=mock_load_config.return_value):
            result_original = runner.invoke(migration_app, [
                "get-original",
                "--config", "test.xml",
                "--type", "address",
                "--name", "test-obj",
            ...
            result_migrated = runner.invoke(migration_app, [
                "get-migrated",
                "--config", "test.xml",
                "--type", "address",
                "--name", "test-obj",
