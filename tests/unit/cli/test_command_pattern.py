...
class TestCommandCompatibility:
    ...
    def test_apply_decorator_to_existing_command(self):
        ...
        with patch('panflow.cli.command_base.CommandBase.load_config') as mock_load_config, \
             patch('panflow.cli.command_base.CommandBase.format_output') as mock_format_output:
...
def test_all_cli_commands_compatibility():
    ...
    command_files = [f for f in os.listdir(cmd_dir) if f.endswith("_commands.py") and not f.startswith("__")]
            ...
            for name, obj in inspect.getmembers(module):
                ...
                if inspect.isfunction(obj) and hasattr(obj, "__typer_function__"):
                    ...
                    try:
                        ...
                        positional_only = any(p.kind == inspect.Parameter.POSITIONAL_ONLY for p in params.values())
                        
                        if config_compatible and context_compatible and not conflicts and not positional_only:
    ...
    compatibility_percentage = (compatible_commands / total_commands * 100) if total_commands > 0 else 0
    print(f"Command compatibility: {compatible_commands}/{total_commands} ({compatibility_percentage:.1f}%)")
    ...
    assert compatibility_percentage > 50, f"Less than 50% of commands are compatible with the new pattern"