...
class TestGraphService:
    ...
    def test_find_objects_by_name_pattern(self, mock_query_executor_class, mock_query_class, 
                                          mock_config_graph_class, graph_service, sample_xml_tree):
    ...
    @patch("panflow.core.graph_service.Query")
    @patch("panflow.core.graph_service.QueryExecutor")
    def test_find_objects_by_value_pattern(self, mock_query_executor_class, mock_query_class, 
                                           mock_config_graph_class, graph_service, sample_xml_tree):
        ...
        expected_query = "MATCH (a:address) WHERE a.value =~ '(?i).*192\\.168\\.1\\..*.*' RETURN a.name"
        mock_query_class.assert_called_once_with(expected_query)
    ...
    @patch("panflow.core.graph_service.Query")
    @patch("panflow.core.graph_service.QueryExecutor")
    def test_find_address_objects_containing_ip(self, mock_query_executor_class, mock_query_class, 
                                               mock_config_graph_class, graph_service, sample_xml_tree):
        ...
        # Call the method
        result = graph_service.find_address_objects_containing_ip(
            sample_xml_tree, 
            "192.168.1"
        )
    ...
    @patch("panflow.core.graph_service.Query")
    @patch("panflow.core.graph_service.QueryExecutor")
    def test_find_service_objects_with_port(self, mock_query_executor_class, mock_query_class, 
                                           mock_config_graph_class, graph_service, sample_xml_tree):
        ...
        # Call the method
        result = graph_service.find_service_objects_with_port(
            sample_xml_tree, 
            "80"
        )
    ...
    @patch("panflow.core.graph_service.Query")
    @patch("panflow.core.graph_service.QueryExecutor")
    def test_find_unused_objects(self, mock_query_executor_class, mock_query_class, 
                                mock_config_graph_class, graph_service, sample_xml_tree):
        ...
        # Call the method
        result = graph_service.find_unused_objects(
            sample_xml_tree, 
            "address"
        )
    ...
    @patch("panflow.core.graph_service.Query")
    @patch("panflow.core.graph_service.QueryExecutor")
    def test_execute_custom_query(self, mock_query_executor_class, mock_query_class, 
                                 mock_config_graph_class, graph_service, sample_xml_tree):
        ...
        result = graph_service.execute_custom_query(
            sample_xml_tree, 
            custom_query
        )
    ...
    @patch("panflow.core.graph_service.Query")
    @patch("panflow.core.graph_service.QueryExecutor")
    def test_filter_objects_by_query(self, mock_query_executor_class, mock_query_class, 
                                    mock_config_graph_class, graph_service, sample_xml_tree):
    ...
    @patch("panflow.core.graph_service.Query")
    @patch("panflow.core.graph_service.QueryExecutor")
    def test_execute_name_query(self, mock_query_executor_class, mock_query_class, 
                               mock_config_graph_class, graph_service):
        ...
        test_cases = [
            ...
            (
                [{"a.name": "test1"}, {"a.name": "test2"}],
                ["test1", "test2"]
            ),
            # Case 2: Results with single column (unnamed)
            (
                [{"column": "test3"}, {"column": "test4"}],
                ["test3", "test4"]
            ),
            # Case 3: Mixed format
            (
                [{"a.name": "test5"}, {"column": "test6"}],
                ["test5", "test6"]
            )
        ]
