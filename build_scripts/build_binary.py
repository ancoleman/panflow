...
def build_binary():
    ...
    cmd = [
        ...
        f"--add-data={os.path.join(ROOT_DIR, 'panflow/xpath_mappings')}:{os.path.join('panflow', 'xpath_mappings')}".replace(
...
if __name__ == "__main__":
    ...
    subprocess.run(
        [sys.executable, "-m", "pip", "install", "-r", os.path.join(ROOT_DIR, "requirements.txt")],
        check=True,
    )
